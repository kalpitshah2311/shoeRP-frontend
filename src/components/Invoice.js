...
const Invoice = ({ showAlert }) => {
    const generatePDF = (data) => {
        ...
        doc.text(20, 110, `Customer name: ${name}`)
        doc.text(20, 130, `Phone number: ${phone}`)
        doc.text(450, 110, `Invoice number: ${data._id}`)
        doc.text(450, 130, `Date: ${Date().slice(0, 10)}`)

        var body = data.products.map((item, index) => {
            return [index + 1, item.brand, item.quantity, item.rate, item.rate * item.quantity]
        })
    ...
    useEffect(() => {
        fetch('https://puzzled-colt-kimono.cyclic.app/api/customer/getallcustomer')
            .then(res => res.json())
            .then(data => {
                setCustomers(data);
            })
    ...
    const handlePrint = async () => {
        ...
        if (!customer) {
            await fetch('https://puzzled-colt-kimono.cyclic.app/api/customer/addcustomer', {
                method: 'POST',
                headers: {
        ...
        fetch('https://puzzled-colt-kimono.cyclic.app/api/invoice/addinvoice', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    const handleAdd = async (e) => {
        ...
        if (brand === '' || size <= 0 || quantity <= 0 || rate === '' || articleNo === '') {
            ...
            await fetch('https://puzzled-colt-kimono.cyclic.app/api/stock/check', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
